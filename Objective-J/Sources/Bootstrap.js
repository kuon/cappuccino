#ifdef CONFIG_RESOLVE_MAIN_BUNDLE_FROM_PAGE_URL
...
GLOBAL(CPApplicationURL) = pageURL;
...
GLOBAL(OBJJ_MAIN_BUNDLE_URL) = mainBundleURL;

#elif defined(CONFIG_RESOLVE_MAIN_BUNDLE_FROM_ARGV)

var root = process.argv.length > 1 ? require('path').dirname(process.argv[1]) : process.cwd();
GLOBAL(OBJJ_MAIN_BUNDLE_URL) = new CFURL("file:" + root + '/');

#elif defined(CONFIG_MAIN_BUNDLE_URL)

GLOBAL(OBJJ_MAIN_BUNDLE_URL) = new CFURL(CONFIG_MAIN_BUNDLE_URL);
